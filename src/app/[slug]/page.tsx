...
export default function BusinessPage({ params }: BusinessPageProps) {
  ...
  return (
    <div 
      className={`min-h-screen transition-all duration-500 ${getBackgroundStyle(theme)} ${getFontClass(theme.font)}`}
      style={{ 
        backgroundColor: theme.style === 'minimal' || theme.style === 'pastel' ? theme.backgroundColor : undefined,
        fontFamily: theme.font === 'caveat' ? 'Caveat, cursive' : undefined
      }}
    >
      {/* Success Banner */}
      {showSuccess && editToken && (
        <div className="bg-green-50 border border-green-200 p-4 m-4 rounded-lg animate-fadeIn">
      ...
      <div className="container mx-auto px-4 py-8 max-w-md">
        ...
        <div className="text-center mb-8 animate-fadeIn" style={{ animationDelay: '0.2s' }}>
          ...
          {/* Business Name */}
          <h1 
            className={`text-4xl font-bold mb-4 transition-all duration-300 ${getTextStyle(theme)}`}
            style={{ 
          ...
          {profile.bio && (
            <p 
              className={`text-lg opacity-90 mb-4 ${getTextStyle(theme)}`}
          ...
          <div className="space-y-4 mb-8">
            {links.filter(link => link.visible).map((link, index) => (
              <button
                ...
                className={`w-full p-4 rounded-lg font-medium transition-all duration-300 hover:scale-105 hover:shadow-lg animate-slideUp ${getButtonStyle(theme)}`}
                style={{
                  backgroundColor: theme.primaryColor,
                  color: theme.style === 'neon' ? theme.textColor : 'white',
                  animationDelay: `${0.1 * index}s`,
                  boxShadow: theme.style === 'neon' ? `0 0 30px ${theme.primaryColor}50` : undefined
                }}
              >
        ...
        {layout.showServices && business.services.length > 0 && (
          <div className="space-y-4 mb-8 animate-fadeIn" style={{ animationDelay: '0.4s' }}>
            <h2 
              className={`text-2xl font-bold text-center mb-6 ${getTextStyle(theme)}`}
              style={{ color: theme.textColor }}
            >
              Our Services
            </h2>
              ...
              {business.services.map((service, index) => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: theme.style === 'glass' ? 'rgba(255,255,255,0.1)' : undefined
                  }}
                >
                  ...
                  <div className={layout.servicesStyle === 'minimal' ? 'flex justify-between items-center w-full' : ''}>
                    <h3 
                      className={`font-semibold ${getTextStyle(theme)}`}
                      style={{ color: theme.textColor }}
                    >
                      {service.name}
                    </h3>
                  ...
                  {layout.servicesStyle !== 'minimal' && (
                    <button
                      ...
                      className={`w-full mt-3 py-2 px-4 rounded-lg font-medium transition-all duration-300 hover:scale-105 ${getButtonStyle(theme)}`}
                      style={{
                        backgroundColor: theme.primaryColor,
                        color: theme.style === 'neon' ? theme.textColor : 'white'
                      }}
        ...
        <div className="space-y-4 mb-8 animate-fadeIn" style={{ animationDelay: '0.6s' }}>
          <button
            ...
            className="w-full py-4 px-6 rounded-lg font-medium transition-all duration-300 hover:scale-105 hover:shadow-lg flex items-center justify-center"
          ...
          <button
            ...
            className={`w-full py-4 px-6 rounded-lg font-medium transition-all duration-300 hover:scale-105 hover:shadow-lg flex items-center justify-center ${getButtonStyle(theme)}`}
            style={{
          ...
          {business.instagram && (
            <button
              ...
              className="w-full py-4 px-6 rounded-lg font-medium transition-all duration-300 hover:scale-105 hover:shadow-lg flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white"
            >
